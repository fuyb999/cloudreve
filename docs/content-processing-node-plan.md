...
- 后台任务清理对话框也已支持直接选择 `content_processing` 聚合类型，便于统一清理历史内容处理任务
- 队列页检查面板与节点页总览均已补充直达内容处理任务视图的入口，形成“队列风险 -> 节点巡检 -> 任务诊断”的运维闭环
- 内容处理任务聚合页顶部也已补充返回队列页与节点页的快捷入口，三类巡检视图之间可双向切换
- 队列页、节点页、任务页均已补充全文索引 / 文档识别 / 媒体元数据 / 缩略图四类内容处理子类型快捷入口，支持从聚合视图继续下钻
- 后台首页已补充内容处理概况卡片，可直接看到节点活跃度、队列状态，并跳转到队列/节点/任务三类巡检入口
- 后台首页与队列页均已补充失败/挂起内容处理任务的直达入口，便于从总览状态直接下钻到异常任务列表
- 后台首页内容处理概况卡片已与队列页共享同一套风险判断逻辑，失败任务、挂起任务、worker 饱和等风险会统一显式提示
- 节点页现已同时展示内容处理节点状态与队列状态，并补充失败/挂起任务入口，使节点巡检页也具备统一风险视图
- 内容处理任务聚合页与子类型页现已支持在“全部 / 失败 / 挂起”状态之间快速切换，便于在任务视图内持续下钻异常范围
- 后台首页、队列页、节点页、任务页现已统一复用内容处理巡检路由、导航入口与任务状态快捷入口组件，减少散落 URL 拼接，形成更一致的巡检 action bar
- 上述巡检入口已进一步合并为统一 `action bar` 组件，首页/队列页/节点页/任务页仅按场景配置开关与文案，后续扩展巡检入口时改动面更可控

...
func (s *settingProvider) OIDC(ctx context.Context) *OIDCSetting {
	return &OIDCSetting{
		...
		Scope:        s.getString(ctx, "oidc_scope", "openid profile email user_info user.read UserInfo.Read Admin.Read Files.Read Files.Write Workflow.Read Workflow.Write Shares.Read Shares.Write"),
	}
}

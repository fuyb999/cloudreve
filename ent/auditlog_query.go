...
func (alq *AuditLogQuery) QueryFile() *FileQuery {
	query := (&FileClient{config: alq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := alq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (alq *AuditLogQuery) QueryEntity() *EntityQuery {
	query := (&EntityClient{config: alq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := alq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (alq *AuditLogQuery) QueryShare() *ShareQuery {
	query := (&ShareClient{config: alq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := alq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (alq *AuditLogQuery) Clone() *AuditLogQuery {
	...
	return &AuditLogQuery{
		...
		withFile:   alq.withFile.Clone(),
		withEntity: alq.withEntity.Clone(),
		withShare:  alq.withShare.Clone(),
...
func (alq *AuditLogQuery) WithFile(opts ...func(*FileQuery)) *AuditLogQuery {
	query := (&FileClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (alq *AuditLogQuery) WithEntity(opts ...func(*EntityQuery)) *AuditLogQuery {
	query := (&EntityClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	alq.withEntity = query
	return alq
}
...
func (alq *AuditLogQuery) WithShare(opts ...func(*ShareQuery)) *AuditLogQuery {
	query := (&ShareClient{config: alq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (alq *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := alq.withFile; query != nil {
		if err := alq.loadFile(ctx, query, nodes, nil,
			func(n *AuditLog, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
	if query := alq.withEntity; query != nil {
		if err := alq.loadEntity(ctx, query, nodes, nil,
			func(n *AuditLog, e *Entity) { n.Edges.Entity = e }); err != nil {
			return nil, err
		}
	}
	if query := alq.withShare; query != nil {
		if err := alq.loadShare(ctx, query, nodes, nil,
			func(n *AuditLog, e *Share) { n.Edges.Share = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (alq *AuditLogQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *File)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuditLog)
	for i := range nodes {
		fk := nodes[i].FileID
...
func (alq *AuditLogQuery) loadEntity(ctx context.Context, query *EntityQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Entity)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuditLog)
	for i := range nodes {
		fk := nodes[i].EntityID
...
func (alq *AuditLogQuery) loadShare(ctx context.Context, query *ShareQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Share)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuditLog)
	for i := range nodes {
		fk := nodes[i].ShareID
...
func (alq *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := alq.ctx.Fields; len(fields) > 0 {
		...
		if alq.withFile != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldFileID)
		}
		if alq.withEntity != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldEntityID)
		}
		if alq.withShare != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldShareID)
		}
	}
